...
@WebServlet("/adoptions_client")
public class AdoptionClient extends HttpServlet {
	private static final long serialVersionUID = 1L;

	/**
	 * @see HttpServlet#HttpServlet()
	 */
	public AdoptionClient() {
		super();
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response)
		...
		List<Chien> chiens = ChienDao.getInstance().getAllEnCoursPerso(idClient);
		...
		if(CompteDao.getInstance().login(email, password)!=null && CompteDao.getInstance().login(email, password).getRole().equals("client")) {
			request.getRequestDispatcher("/jsp/adoptions_client.jsp").forward(request, response);
		}else {
